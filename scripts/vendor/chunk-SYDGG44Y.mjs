import { a as q } from "chunk-4RSUK7L6.mjs";
import { a as S, b as C } from "chunk-3FCHXJA6.mjs";
import { a as w } from "chunk-AHA23F7Q.mjs";
import { Va as y } from "chunk-SC3EIR2W.mjs";
import { ha as g } from "chunk-P3M2JHXY.mjs";
import { c as j, d as k } from "chunk-DQNSEOJA.mjs";
import { c as $ } from "chunk-LTBNROLY.mjs";
import { b as o } from "chunk-FUUVFP2C.mjs";
import { q as M } from "chunk-F4RHXZMY.mjs";
import { o as P } from "chunk-HSJM72PS.mjs";
import { m as R } from "chunk-LA34HORX.mjs";
import { a as O } from "chunk-W774OHJB.mjs";
import { k as A } from "chunk-BSTISVF5.mjs";
import { Ra as l, ia as b } from "chunk-FCG35XJJ.mjs";
import { b as v, c as I } from "chunk-4JY5UMT2.mjs";
  ...
  h = class {
    ...
    getACL() {
      o.get(`/web/projects/${this.projectId}/acl/`, {
        ...
        .catch((e) => u.error("Failed to get ACL:", e));
    }
    setInitialProject(e) {
      e.then((s) => {
        this.dispatch({ type: "setProject", project: s });
      }).catch((s) => u.error("Failed to set initial project:", s));
    }
    getProject() {
      o.get(`/web/projects/${this.projectId}`, {
        ...
        .catch((e) => u.error("Failed to get project:", e));
    }
    async pollProject({ intervalMillis: e, attempts: s, stopCondition: t }) {
      let r = await w(this.projectId, { intervalMillis: e, attempts: s, stopCondition: t });
      return (
        r.status === 0 &&
    ...
    async invite(e) {
      let s = await S(this.projectId, e);
      return (
        s.status === 0 &&
    ...
    async updateUserPermissions(e) {
      let s = await C(this.projectId, e);
    ...
    async updateProject(e, s = !0) {
      ...
      try {
        (await $(this.projectId, e), this.getProject(), this.notifyProjectChange("metadata"));
      } catch (t) {
        throw (u.error("Failed to update project:", t), t);
      }
    }
    ...
    async uploadAssetByURL(e, s = l) {
      let t = await U(`/web/projects/${this.projectId}/assets/fetch`, e, "image", s);
      return (t && this.notifyProjectChange("assets"), t);
    }
    async duplicateAssets(e, s) {
      if (this.projectId === s)
        return (u.warn("Attempted to duplicate assets for current project"), []);
    ...
    async addModuleRequestToForm(e, s) {
      ...
      (y && window.CompressionStream && (r.transferEncoding = "gzip"),
        s.append(
          "metadata",
          JSON.stringify({
            ...r,
        ...
        await Promise.all(
          t.map(async (i) => {
            ...
            v(!R(c), "File needs content or bytes");
            let d = new Blob([c]);
            if (y && window.CompressionStream) {
              let E = new window.CompressionStream("gzip"),
                T = d.stream();
              d = await new Response(T.pipeThrough(E)).blob();
              let f = c.length - d.size,
                N = (f / c.length) * 100;
              u.debug("Saved", f, "bytes", `(${N.toFixed(1)}%)`, "compressing", i.name);
            }
...
function le(n, e, s) {
  let t = (0, p.useMemo)(() => new h(n, e.projectId, s), [n, e.projectId, s]);
  (0, p.useEffect)(() => {
    t.getACL();
  }, [t]);
  let r = (0, p.useRef)(e);
...
export { L as a, h as b, le as c, ge as d, fe as e };
//# sourceMappingURL=chunk-SYDGG44Y.mjs.map
