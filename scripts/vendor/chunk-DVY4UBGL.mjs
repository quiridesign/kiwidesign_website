import { A as bj, B as X, C as gj, D as Oj, E as EA } from "chunk-SC3EIR2W.mjs";
import { S as Sj } from "chunk-G33TSN2Y.mjs";
import {
  $a as oj,
  Ef as U,
  Ff as Rj,
...
import { d as V } from "chunk-JZSLOVR5.mjs";
import { b as P, c as nj } from "chunk-2FANYRRP.mjs";
import { a as fj, h as Q } from "chunk-RHLSOZ2K.mjs";
import { B as $j } from "chunk-BNJBFYWR.mjs";
import { a as _j } from "chunk-OCWO35R2.mjs";
import { Aa as rj, xa as ej, xb as kj, za as tj } from "chunk-YEI3MERI.mjs";
import { Ob as L } from "chunk-PJ6FAON2.mjs";
import { f as R } from "chunk-GWGM5D6M.mjs";
import { a as z } from "chunk-WCTCEL2Q.mjs";
import { a as $A } from "chunk-4QQP7OCQ.mjs";
import {
  $k as vA,
  KG as zA,
  KJ as H,
...
import { b as LA } from "chunk-MG2W7LMV.mjs";
import { a as Bj } from "chunk-F3T3XR66.mjs";
import { b as c, d as kA, e as D, k as DA, m as T, n as TA } from "chunk-LA34HORX.mjs";
import {
  Ba as S,
  Da as PA,
  X as C,
...
import { b as l, c as I } from "chunk-4JY5UMT2.mjs";
...
async function D6({ localId: A, name: j, source: e, includeSourceMap: r = !0 }) {
  let t = await v();
  try {
    let {
        code: k,
...
async function q6({
  ...
  try {
    let {
      ...
      { moduleCode: b, annotations: m, exportedNames: E } = yj(B, $, t);
    return {
      code: b,
      sourceMap: s ?? void 0,
...
function X6(A, j, e, r, { shouldOpenPage: t = !0, renamedIds: k = new f() } = {}) {
...
function Qj(A, j, e) {
  let r = new f(),
    ...
    a = j.clone({ name: k, children: new vA() });
  r.set(j.id, a.id);
  let i = [];
...
function Z(A, j) {
  ...
  if (e) return e;
  let r = S(j);
  return !O(r) || !hA(r) ? null : A.componentForIdentifier(OA(r));
}
var Wj = 0;
async function AA(A, j, e = Wj) {
  if (e === 0) {
    let t = S(j),
      k = U(A.stores.treeStore, A.stores.modulesStore, A.stores.pluginStore, t);
...
async function qj(
  ...
  if (B.ok) {
    ...
    for (let d of B.value.relative) {
      let g = wA(d.specifier, `${e.module.type}/`);
      if (!g) {
    ...
    for (let d of s.imports) {
      ...
      let g = `${d.moduleType}/${d.moduleName}`,
        y = E.get(g);
      if (!y) {
        K.warn("Unable to get source location of import", g, "when unlinking");
        continue;
      }
      for (let fA of y) {
        p ||
  ...
  return (
    r &&
      (await a(() => {
        ...
        E && E.set({ codeComponentIdentifier: m });
      })),
    t && ((A.stores.chromeStore.contentPanelTab = "Assets"), AA(A, m, k)),
    m
  );
}
...
async function FA(
  ...
  l(n, "external module must exist on preload");
  let B = {
      identifier: j,
      info: n,
      instance: r,
...
async function pA(A, j) {
  let { instance: e, withUnlinkModal: r, ...t } = j,
    k = Q(A.stores.treeStore.tree, j.identifier),
...
function j7({ treeStore: A, stackElement: j }) {
...
function r7(A, j = []) {
  ...
  if (a) {
    ...
    if (t.type === "existing")
      return t.itemId === a.id
        ? r
        : (l(r.length === 1, "Must dismiss overlay stack before changing selection"), [Cj(A, a)]);
  ...
  if (t.type === "existing")
    return A.stores.chromeStore.mainView !== 2
      ? r
      : (l(r.length === 1, "Must dismiss overlay stack before de-selecting collection item"), []);
  if (t.type === "arrayItem") return r;
  I(t);
}
function e6(A, j) {
...
function jA(A, j, e) {
  ...
  l(j.parentid, "Existing collection item must have a parent collection");
...
function r6(A, j) {
  let e = A.stores.insertSidebarStore.getEditBehavior(j.moduleId),
    r = Q(A.stores.treeStore.tree, j),
    t = A.componentLoader.componentForIdentifier(j.value)?.annotations;
  return (r && e === "block") || yA(t, "framerDisableUnlink");
}
function Jj(A, j, e, r) {
  let t = S(A),
    k = w(j, "canDesign"),
    { includeAccelerator: a = !0 } = r;
  if (O(t) && (t.type === "canvasComponent" || t.type === "screen" || t.type === "vector"))
    return [
      {
        label: k ? "View" : "Edit",
        accelerator: a ? V.fakeSelectChildren : void 0,
        click: () => {
          if (t.type === "vector") {
  ...
  if (SA(t) && t.kind === "externalModuleExport") {
    ...
    let s = j.stores.insertSidebarStore.getEditBehavior(t.moduleId),
      $ = j.stores.treeStore.tree.getNode(t.moduleId),
      m = r.externalModuleType === "canvasComponent" && $ && $.ownerId === "1h0OTH9KlHTd4pyc9CLt";
    if (r6(j, t)) return u();
    if (!RA(r.externalModuleType)) return u();
  ...
  if (!i || i.type !== "component" || i.depth !== 0) return u();
  let o = r.keepHistory
      ? void 0
      : () => {
          j.stores.scopeStore.goBack();
        },
    n = U(j.stores.treeStore, j.stores.modulesStore, j.stores.pluginStore, t);
  return [
    ...(n && Fj(j.stores.modulesStore, t) && !k && O(A)
      ? [
          {
            label: Rj(n),
            click: () => {
...
function o6({ collectionId: A, engine: j, saveAction: e, source: r }) {
  let t = j.tree.getNodeWithTrait(A, _);
  l(t, "Collection needs to exist to allow creation of new item");
  let k = j.stores.selectionStore.ids[0],
    a = j.tree.getNodeWithTrait(k, _);
  (a && a.id !== t.id && l(e, "saveAction is required when creating a nested private draft item"),
    n6({ collection: t, engine: j, saveAction: e }),
    R("collection_record_create", { source: r }));
}
function n6({ collection: A, engine: j, saveAction: e }) {
  let r = s6(j, A, 0);
...
function s6(A, j, e) {
  let r = A.stores.sessionStore.user.id,
    t = $6(j, r);
  if (t) return t;
  let k = rj.create({ privateToUserId: r, ...ej(A.tree.root.locales ?? u()) });
  return (A.stores.scopeStore.insertNode(k, j.id, e), B6(j, k), xj(A) && k.set({ isDraft: !0 }), k);
}
function $6(A, j) {
  let e,
    r = 0;
...
function B6(A, j) {
  ...
  for (let r of A.variables)
    r.type === "date" &&
      j.setControlProp(r.id, { type: "date", value: uA(e, r.options?.displayTime) });
}
function E6(A) {
...
function c6(A, j, e) {
  ...
  return Number.isNaN(r.getTime()) ? !1 : j.value === uA(r, e.options?.displayTime);
}
function _A(A, j, e) {
  if (!A) return !1;
  let r = A.get(j);
...
function d6(A, j, e) {
  if (j.type === "slug") return (r) => _A(A.get(j.id), r, e);
}
function wj(A, j, e, r, t, k, a) {
  if (t === "associatedVariable" && A === "string") {
    ...
    if (!r || !sA(j)) return;
    let { maxLength: n } = j;
    return D(n) && r.length > n ? { type: "max-length", message: X(r.length, n) } : void 0;
  }
  if (A === "slug") {
    let n;
    if ((e && kA(e.value) ? (n = M(e.value)) : r && (n = M(r)), !n)) return { type: "required" };
    if ((l(a, "isSlugTaken is required for slug fields"), a(n)))
...
function G6(A, j, e) {
  ...
  switch (r) {
    ...
    case "richtext":
      return A
        ? (l(A.type === "richtext"),
          l(!q(A.value), "Dynamic values are not supported within the CMS."),
    ...
    case "responsiveimage": {
      ...
      return A
        ? (l(A.type === "image"), !A.value && !t ? { type: "required" } : void 0)
        : t
          ? void 0
          : { type: "required" };
    }
    case "string":
      return A
        ? (l(j.type === "string"), !kA(A.value) ? { type: "required" } : void 0)
        : j.defaultValue
          ? void 0
          : { type: "required" };
    ...
    case "array": {
      ...
      let t = XA(j, A);
      return (
        l(!q(t), "Dynamic values are not supported within the CMS."),
  ...
  if (D(e))
    switch (j.type) {
      case "string": {
        ...
        return t.length > e ? { type: "max-length", message: X(t.length, e) } : void 0;
      }
      case "richtext": {
        if (!A || q(A.value)) return;
        let r = $j(A.value);
        return r.length > e ? { type: "max-length", message: X(r.length, e) } : void 0;
      }
      default:
        I(j);
    }
...
function R6(A, j, e) {
  if (!WA(j) || (TA(A?.value) && !NA(A?.value)) || qA(A?.value)) return;
...
function lt(A, j, e, r) {
  ...
  return k.filter((i) => {
    ...
    if (!o) return !1;
    let n = d6(a, i, r.id);
    return !!F6(A, r, i, e.variables, o, n, r.getControlProp(i.id));
  });
}
function Uj(A, j) {
  let e = M(A);
...
function It(A, j, e, r) {
  for (let t of A.variables) {
    ...
    if ((!k || !c(k.value) || !k.value.length) && t.associatedStringVariable) {
      ...
      if (i && c(i.value)) {
        let o = Uj(i.value, (n) => _A(e.get(t.id), n, j.id));
        if (o.length)
          return (
...
//# sourceMappingURL=chunk-DVY4UBGL.mjs.map
