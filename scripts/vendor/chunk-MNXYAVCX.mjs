import { b as I, c as R } from "chunk-JZSLOVR5.mjs";
import { a as P } from "chunk-JTCAKYEM.mjs";
import { a as y, b as C, m as g, o as b } from "chunk-LA34HORX.mjs";
import { a as z } from "chunk-W774OHJB.mjs";
import { j as W } from "chunk-SPICCGE6.mjs";
import { ia as O, q as S } from "chunk-FCG35XJJ.mjs";
...
function V(t, e, s = []) {
  ...
  for (let [o, n] of t.entries()) {
    ...
    if (n.role && e?.validateAction) {
      let { states: u, options: f } = e.validateAction(n.role),
        l = u[n.role],
        { label: A, checked: h, visible: M } = f[n.role] ?? {};
      (C(A) && (r.label = A),
        g(n.checked) && y(h) && (r.checked = h),
        g(n.visible) && y(M) && (r.visible = M),
        g(n.enabled) && y(l) && (r.enabled = l));
    }
    if (n.visible === !1) continue;
    if (a) {
      let u = b(a) ? q(a, e, p) : V(a, e, p),
        f = { ...r, submenu: u };
      i.push(f);
      continue;
    }
...
function G(t, e) {
  ...
  let n = { ...t };
  return (
    !S() && !t.acceleratorWindows && (n.acceleratorWindows = R(o) ? i[o] : void 0),
    t.accelerator || (n.accelerator = I(o) ? s[o] : void 0),
    n
  );
}
var J = O("wantsNativeTextActions");
function L(t = document.activeElement) {
  let e = N(t);
  if (!e) return !1;
  ...
  if (e instanceof HTMLInputElement)
    switch (e.type) {
      ...
      case "submit":
        return !1;
      default:
        return (J.reportErrorOncePerMinute("Unhandled input type: " + e.type), !1);
    }
  ...
  class s {
    ...
    then(n) {
      ...
      let a = this.actionToTargetMap,
        d = n.wantedActions;
      for (let p of d) a[p] = a[p] || n;
      return this;
    }
    wantedActions = [];
    wantsAction(n, a) {
      let d = this.actionToTargetMap[n];
      return !d || !x(d) ? !1 : k(d, n, a);
    }
    optionsForAction(n) {
...
function it({ target: t, engine: e, children: s, style: i, global: o }) {
  let n = (0, m.useRef)(null),
    a = (0, m.useCallback)(
      (r) => {
        let c = r.actionIdentifier;
        if (!c) return;
        let u = D.functionTarget(t, c);
        if (!u || !u.wantedActions.includes(c) || !x(u)) return;
        let f = u[c];
        if (!b(f)) return;
        let l = k(u, c, r);
        ...
        e.setEditReason(c);
        let A = f.bind(u);
        e.scheduler.wrapHandler(A)(r.actionArgument);
      },
      [e, t]
    ),
    d = (0, m.useCallback)(
      (r) => {
        let { invalidatedActions: c, validatedActionStates: u, validatedActionOptions: f } = r;
        if (!(!c || !u || !f) && t) {
  ...
  (0, m.useEffect)(() => {
    ...
    return (
      ...
      r.addEventListener(v.eventType, d),
      () => {
        (r.removeEventListener(T.eventType, a), r.removeEventListener(v.eventType, d));
      }
    );
  }, [o, a, d]);
  let p = (0, m.useCallback)((r) => {
    !L(r.target) && !W.isDebugBuild && r.preventDefault();
  }, []);
  return (0, j.jsx)("div", { ref: n, onContextMenu: p, style: i, children: s });
}
function st(t, e) {
...
function ct(t) {
  let e = t === "*" ? Object.keys(w) : [t];
  U.forEach((o) => o());
  let { states: s, options: i } = K(e);
  (Object.assign(w, s), Object.assign(H, i));
  for (let o of e) w[o] = s[o] || (o === "undo" || o === "redo" || o === "delete" ? "native" : !1);
  return (B.forEach((o) => o()), { states: w, options: H });
}
var w = {},
  H = {};
export { L as a, $ as b, D as c, it as d, st as e, rt as f, at as g, ct as h, V as i };
//# sourceMappingURL=chunk-MNXYAVCX.mjs.map
