import { a as Te, d as Ht, e as $t, f as Ut } from "chunk-L5UZHYBP.mjs";
import { A as Yt, i as Zt } from "chunk-DVY4UBGL.mjs";
import { e as qt } from "chunk-G33TSN2Y.mjs";
import {
  Jh as Kt,
  Nb as Rt,
  Ob as At,
...
import { i as zt, n as Bt } from "chunk-TACLCPXO.mjs";
import { n as Wt, o as Dt, q as Gt } from "chunk-TNHWGVQK.mjs";
import { b as x } from "chunk-2FANYRRP.mjs";
import { r as jt } from "chunk-F4RHXZMY.mjs";
import { h as Xt } from "chunk-RHLSOZ2K.mjs";
import { W as ae, a as vt, ae as Vt, b as It, x as xt, yc as Et } from "chunk-YEI3MERI.mjs";
import { Kc as wt, Oa as Ct, w as ft } from "chunk-PJ6FAON2.mjs";
import { f as w } from "chunk-GWGM5D6M.mjs";
import {
  $k as Ke,
  Ac as We,
  Be as K,
...
import { da as Ce, ha as Je, oa as Qe } from "chunk-DHUNMEBO.mjs";
import { o as Oe } from "chunk-HSJM72PS.mjs";
import { b as Le } from "chunk-AFIDN3ND.mjs";
import { e as ne } from "chunk-LA34HORX.mjs";
import { Ba as Ae, X as te, ia as oe, sa as j, ta as re } from "chunk-FCG35XJJ.mjs";
...
var Qt = oe("addImagesToCanvas"),
  No = /\.[^.]+$/u;
async function xr(e, t, o, r, n = !1, i = !1, d) {
  if (!t || t.length === 0) return [];
  let m = d ?? bo(e.stores, t.length, o, n),
    s = new qt(e, { silent: i }),
    a = new Set(),
    p = [];
  for (let l of t)
    if (l instanceof File && l.type === Qe.get("svg")) {
      let F = await l.text();
      Gt(F) ? (s.add(l), a.add(l.name)) : p.push(l);
    } else s.add(l);
  let f = (await s.results()).filter(zt),
    u = e.scheduler.wrapHandler(Eo(e)),
    g = [],
  ...
  for (let { filename: l, originalFilename: F, imageSize: E } of f) {
    let L = a.has(F) ? "svg" : "bitmap",
      { nonZeroNaturalWidth: b, nonZeroNaturalHeight: O } = Wt(E);
    g.push(
      ...u(
        l,
        F,
  ...
  if (p.length) {
    if (p.length === 1) return await xo(e, p[0], r, m);
    Ue(h.active)
      ? v.set({
          type: "VectorImport",
          svgsToImport: p,
          scopeId: h.active.id,
          currentVectorLayout: So(h.active),
          source: "canvas",
        })
      : v.set({
...
function xo(e, t, o, r) {
  return t ? ko(e, t.name, t, r, o) : [];
}
function bo(e, t, o, r) {
  let { selectionStore: n, scopeStore: i, treeStore: d } = e,
    m = { type: "node", parentId: Nt(d.tree, i.active, [], o), position: o };
...
function Eo(e) {
  return (t, o, r, n, i, d) => {
    ...
    for (let a of i) {
      ...
      if (p.type === "node") {
        let f = ie(),
          { baseAttributes: u } = eo(e, p, f, n, o);
        Qt.info("create image:", o, "size:", n);
        let g = {
          ...u,
...
function Vo(e) {
  return (t, o, r, n, i, d) => {
    let m = e.tree,
      s = [];
    for (let a of n) {
      Qt.info("create svg:", o, "size:", r, "bytes:", t.length, "colorable?", d);
      let p = a.type === "node" ? a : { type: "node", parentId: a.node.id },
        f = ie(),
        { constraints: u } = eo(e, p, f, r, o);
      f = Te(m, p.parentId, () => {
        let v = Lt(t, Vt.createEmpty, r);
...
function eo(e, t, o, r, n) {
  let i = t.parentId,
    d = t.position ? Tt(e, i, t.position, r) : void 0,
    m = n.replace(No, "");
  return {
    baseAttributes: {
      name: wo(m),
...
async function ko(e, t, o, r, n) {
  ...
  try {
    let m = await Fo(o),
      s = Dt(m),
      p = e.scheduler.wrapHandler(Vo(e))(m, t, s, r, n, !0);
...
async function Fo(e) {
  ...
  let o = await fetch("/internal/svgo", { method: "POST", body: t, headers: jt });
...
var Ao = {
  ...
  de = "external_",
  Se = "local_",
  no = "legacy_design_component",
  io = "legacy_code_component",
  so = `${Se}${io}`,
  Lo = `${de}${io}`,
  ao = `${Se}${no}`,
  mo = `${de}${no}`;
var Oo = new Set(["form", "frame", "image-frame", "stack", "repeater"]),
  _o = (e) => e.startsWith(Ne),
  co = (e) => e === ao || e === mo,
  Mo = (e) => Oo.has(e);
function lo({ canvasNode: e, source: t, isDrag: o, engine: r }) {
  let n = Wo(e, r);
  ...
  if (
    ...
    co(n.type) && w("component_design_instance", {}),
...
function Wo(e, t) {
  if (A(e)) {
    ...
    if (!i)
      return {
        type: e.codeComponentIdentifier.startsWith(".") ? so : Lo,
    ...
    if (j(i)) {
      ...
      return Xt(t.stores.treeStore.tree, n)
...
function Do({ type: e }) {
  return e.startsWith("local_module_") || e === so
    ? "code_component"
    : co(e)
      ? "design_component"
      : e.startsWith(de)
...
function xe(e) {
  let t = new wt({ widthType: 2, heightType: 2, html: "<p>No items</p>" });
  return (
    ...
    new Ct({
      ...
      radius: 20,
      children: new Ke([t]),
      visible: _e("boolean", { type: "variableReference", id: gt, providerId: e.getPrimaryId() }, [
...
function zo(e, t, o) {
  ...
  let s = uo(d, n),
    a = yo(d, n),
    p = uo(m, o.dataIdentifier),
    f = yo(m, o.dataIdentifier);
  if (f)
    return {
      id: V(),
      itemKey: "id",
...
function $o(e, t, o, r = !1) {
  ...
  return (r && (i = i.filter(z)), (i = ct(i, n).reverse()), i.length > 0 && i[0] ? i[0] : null);
}
var go = { width: 1, widthType: 1, heightType: 2, left: 0, right: null };
function Co({
  engine: e,
  component: t,
  canvasPoint: o,
  ...
  let l = f ? e.tree.getNode(f) : (I ?? $o(e, t, o, d)),
    F = u.ids.length === 0,
    E = v.active,
  ...
  if ((m && d && L && F && (l = L), a === 1 && l && se(c) && !z(l))) {
    ...
    let C = l.getPrimaryId(),
      y = Kt(e.componentLoader, e.stores, "framer/useShowRelativeOverlay", C, c.draftOrCurrent());
    if (!y) return;
    e.tree.move(y.id, C);
    return;
  } else se(c) && c.set(st);
  ...
  if ((ho(e, l ?? v.active, t), I)) {
    let C = be(e, t);
    if (f) b = I.children.length;
    else {
      let { insertionIndex: y, wrap: T } = Ht(I, e, o, C);
      (T && (O = T), ne(y) && (b = y));
    }
    if (m && d) {
      ...
      let { zoom: B } = g,
        ...
        U = $ / (y.width || 1),
        ee = pe / (y.height || 1),
        vo = Math.min(U, ee, 0.5),
        Io = N.center(y);
      g.scrollToCenter(Io, { animate: !0, zoom: vo });
    }
    At(e, I, t, h);
  }
  let ce;
  if (l) {
    ...
    ce = { ...pt(e.tree, l, h), width: C, height: y };
  } else ce = h;
  let J = c.updateForRect({
    rect: ce,
    parentSizeInfo: null,
    constraintsLocked: !1,
    shouldGuessPinToBottom: l ? vt(e.tree, c, l) : !0,
    shouldGuessPinToRight: l ? It(e.tree, c, l) : !0,
  });
  (l && G(l) && Z(c) && K(c) && (J = jo(c, J)), oo(e.componentLoader, c, J), c.set(J, e.tree));
  let we = c.id,
    Ee = !1,
    P = null;
  if (l) {
...
var qo = oe("clipboard:modules");
async function Ci(e, t, o) {
  if (!t || !yt(t)) return !1;
  let r = "insert-external-component";
  try {
    (await Xo({ engine: e, moduleURL: t, position: o, centerInParent: !0, toastKey: r }),
      x({ type: "remove", key: r }));
  } catch (n) {
...
async function Xo({
  ...
  if (s.type === "screen")
    ...
    let u = await Zt(e, { identifier: p, moduleType: s.type, enterIsolation: !1 });
...
//# sourceMappingURL=chunk-TFHHJRNA.mjs.map
